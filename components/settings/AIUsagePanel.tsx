 ...
 * Uses useUsageStore for state management (Zustand).
 */
...
export function AIUsagePanel() {
    ...
    const loadData = useCallback(() => {
        setSummary(getUsageSummary(getRecordsForMode(viewMode)));
        const credit = getCreditBalance('perplexity');
        setPerplexityCredit(credit?.remaining ?? 5.00);
    }, [viewMode, getRecordsForMode, getUsageSummary, getCreditBalance]);

    const handleViewModeChange = useCallback((mode: 'session' | 'today') => {
        setViewMode(mode);
        setSummary(getUsageSummary(getRecordsForMode(mode)));
    }, [getRecordsForMode, getUsageSummary]);
