 ...
 * Uses usageStore (Zustand) for state management.
 */
...
export default function CostDashboard({ compact = false, onClose }: CostDashboardProps) {
    ...
    const {
        ...
        getSessionUsage,
        getCreditBalance,
        setCreditBalance: storeSetCreditBalance,
        clearUsageRecords,
    } = useUsageStore();
    ...
    // Compute records based on view mode
    const getRecordsForMode = useCallback((mode: 'session' | 'today') => {
        return mode === 'today' ? getTodayUsage() : getSessionUsage();
    }, [getTodayUsage, getSessionUsage]);

    // Initialize summaries lazily
    const [summary, setSummary] = useState<UsageSummary>(() =>
        getUsageSummary(getRecordsForMode('session'))
    );
    const [todaySummary, setTodaySummary] = useState<UsageSummary>(() =>
        getUsageSummary(getRecordsForMode('today'))
    );

    // Get initial credit
    const creditData = getCreditBalance('perplexity');
    const [perplexityCredit, setPerplexityCredit] = useState<number>(creditData?.remaining ?? 5.00);

    const loadData = useCallback(() => {
        setSummary(getUsageSummary(getRecordsForMode('session')));
        setTodaySummary(getUsageSummary(getRecordsForMode('today')));
        const credit = getCreditBalance('perplexity');
        setPerplexityCredit(credit?.remaining ?? 5.00);
    }, [getRecordsForMode, getUsageSummary, getCreditBalance]);

    const handleSaveCredit = () => {
        const value = parseFloat(creditInput);
        if (!isNaN(value) && value >= 0) {
            storeSetCreditBalance('perplexity', value, 5.00);
